...
func (s *AdmissionService) CreateAdmission(ctx context.Context, patientID uuid.UUID, bedID int, admissionDiagnosis string, treatment string) (*domain.Admission, error) {
	...
	err = tx.QueryRow(ctx, insertQuery,
		admission.PatientID, admission.BedID, admission.Status, admission.EventType, admission.AdmissionDiagnosis, admission.CurrentDiagnosis, admission.Treatment,
...
// UpdateTreatment updates the treatment of an active admission
func (s *AdmissionService) UpdateTreatment(ctx context.Context, admissionID uuid.UUID, treatment string) (*domain.Admission, error) {
	// Verify admission exists
	admission, err := s.admissionRepo.GetByID(ctx, admissionID)
	if err != nil {
		return nil, domain.ErrAdmissionNotFound
	...
	// Update in database
	err = s.admissionRepo.UpdateTreatment(ctx, admissionID, treatment)
	if err != nil {
		return nil, err
	}
